...
/*   Updated: 2019/04/23 15:48:17 by vice-wra         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../ft_printf.h"

char *put_bignum_strings_into_one(t_bignum *num, t_fs *form_string, char sign)
{
	char *str;
	...
	if (form_string->precision == 0)
		...
		str = ft_strjoin(cust_strjoin_left(&num->int_part, "."), ft_strsub(cust_strdup(&num->frac_part), 0, form_string->precision));
	if (sign == '-')
		add_sign(&str, '-');
	else if (sign == '+')
		add_sign(&str, '+');
	if (form_string->width > ft_strlen(str))
